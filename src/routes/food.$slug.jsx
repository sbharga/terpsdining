...
import { useLoaderData, useFetcher, useNavigate, Await } from 'react-router';
...
export async function loader({ params }) {
  ...
  const foodPromise = getFoodBySlug(params.slug);
  const dataPromise = foodPromise.then(food =>
    Promise.all([
      getFoodMenuHistory(food.id),
      getFoodRatings(food.id),
      getTodayAppearancesForFood(food.id),
      user ? getUserRatingForFood(user.id, food.id) : null
    ]).then(([history, ratings, todayAppearances, userRating]) => ({ food, history, ratings, todayAppearances, userRating }))
  );

  return { data: dataPromise, user };
}
...
export async function action({ request }) {
  ...
  await upsertRating(user.id, formData.get('foodId'), {
    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <label key={n} className="cursor-pointer">
          ...
          />
          <span
            className={`text-2xl leading-none select-none transition-colors ${n <= value ? 'text-accent' : 'text-gray-300 hover:text-accent/60'
              }`}
          >
            ★
          </span>
...
function buildBreakdown(ratings) {
  ...
  const avg = (key) => {
    ...
    return vals.length ? (vals.reduce((a, b) => a + b, 0) / vals.length).toFixed(1) : '&mdash;';
  };
  return {
...
function buildDailyChart(history) {
  const hallsPerDay = {};
  history.forEach(({ date, dining_halls }) => {
    if (!hallsPerDay[date]) hallsPerDay[date] = new Set();
    if (dining_halls?.slug) hallsPerDay[date].add(dining_halls.slug);
  });
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    days.push([key, hallsPerDay[key] ? hallsPerDay[key].size : 0]);
  }
  return days;
}
...
export default function FoodPage() {
  ...
  return (
    <Suspense fallback={<FoodSkeleton />}>
      <Await resolve={data}>
        {({ food, history, ratings, todayAppearances, userRating }) => {
...
function FoodSkeleton() {
  return (
    <div className="space-y-8 max-w-2xl animate-pulse mt-8">
      ...
      <div className="space-y-3">
        ...
        <div className="h-32 bg-gray-200 rounded max-w-sm" />
      </div>
    </div>
  );
}
